...
Sub Test_UnlockProject()
    Select Case UnlockProject(ActiveWorkbook.VBProject, "LM")
        Case 0: If Debug_Warn Then Debug.Print "The project was unlocked"
        Case 2: If Debug_Warn Then Debug.Print "The active project was already unlocked"
        Case Else: If Debug_Warn Then Debug.Print "Error or timeout"
    End Select
End Sub
...
#End If
Dim timeout As Date
Dim vbext_pp_locked As Integer
    ...
    If lRet = 0 Then
        If Debug_Warn Then Debug.Print "error setting timer"
        GoTo ErrorHandler
    End If
...
Dim pwd As String
    ...
    Do While Now() < timeout
        ...
        If Debug_Warn Then Debug.Print "found window"
        ...
        If Debug_Warn Then Debug.Print "hwndpassword: " & hWndPassword

        ' Get the handle for the OK button
        hWndOK = GetDlgItem(hWndTmp, IDOK)
        If Debug_Warn Then Debug.Print "hwndOK: " & hWndOK
...
ErrorHandler:
    If Debug_Warn Then Debug.Print Err.Number
...
Dim sCaption As String
    ...
    If Debug_Warn Then Debug.Print sCaption
    ...
    Do While Now() < timeout
        ...
        ' Found the dialog box, make sure it has focus
        If Debug_Warn Then Debug.Print "found properties window"
        lRet2 = SendMessage(hWndTmp, TCM_SETCURFOCUS, 1, ByVal 0&)
